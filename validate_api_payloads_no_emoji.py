...
def validate_har_files():
    ...
    if not har_files:
        print("ERROR: No HAR files found!")
        return False
    
    print(f"SUCCESS: Found {len(har_files)} HAR files")
    ...
    try:
        ...
        for endpoint, description in endpoints.items():
            if endpoint in content:
                found_endpoints.append(f"SUCCESS: {description}: {endpoint}")
            else:
                found_endpoints.append(f"ERROR: {description}: {endpoint}")
        ...
        for cookie in ['JSESSIONID', 'ticketmv', 'SERVERID']:
            if cookie in content:
                cookies_found.append(f"SUCCESS: {cookie}")
            else:
                cookies_found.append(f"ERROR: {cookie}")
        ...
        print(f"ERROR: HAR file analysis failed: {e}")
        return False
...
def validate_reservation_payload():
    ...
    try:
        ...
        print("SUCCESS: Reservation payload construction successful")
        print(f"   Sample payload keys: {list(test_payload.keys())}")
        
        return True
        
    except Exception as e:
        print(f"ERROR: Reservation payload validation failed: {e}")
        return False
...
def validate_session_management():
    ...
    try:
        ...
        proxy = _get_proxy()
        if proxy:
            print(f"SUCCESS: Proxy acquisition: {proxy}")
        else:
        ...
        print(f"ERROR: Session management validation failed: {e}")
        return False
...
def validate_database_integration():
    ...
    try:
        ...
        if created:
            print("SUCCESS: Sniper task creation successful")
            sniper_task.delete()  # Clean up
        else:
            print("SUCCESS: Sniper task already exists")
    ...
    try:
        ...
        if hasattr(settings, 'CELERY_BEAT_SCHEDULE'):
            ...
            if 'instant-sniper-scan' in schedule:
                ...
                print("SUCCESS: Instant sniper configured in Celery beat")
                ...
                print("ERROR: Instant sniper not configured in Celery beat")
                return False
        
        return True
        
    except Exception as e:
        print(f"ERROR: Celery configuration validation failed: {e}")
        return False
