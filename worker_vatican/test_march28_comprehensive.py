...
class March28TicketTester:
    ...
    async def test_with_curl_method(self):
        ...
        try:
            ...
            # Get ticket IDs using curl method
            ticket_ids = await self.monitor._get_ticket_ids(self.test_date, self.test_visitors)
            logger.info(f"📋 Found {len(ticket_ids)} ticket IDs via curl: {ticket_ids}")
            
            # Check availability for each ticket ID
            all_slots = []
            for ticket_id in ticket_ids:
                ...
                if slots:
                    ...
                    logger.info(f"✅ Ticket ID {ticket_id}: Found {len(slots)} slots")
                else:
                    logger.info(f"❌ Ticket ID {ticket_id}: No slots found")
    ...
    async def test_with_playwright_method(self):
        ...
        try:
            ...
            # Get ticket IDs again (should be fresh from browser session)
            ticket_ids = await self.monitor._get_ticket_ids(self.test_date, self.test_visitors)
            logger.info(f"📋 Found {len(ticket_ids)} ticket IDs via Playwright: {ticket_ids}")
    ...
    async def test_direct_api_calls(self):
        ...
        logger.info("🔄 Testing direct API calls with various parameters")
        
        try:
            # Test with different visitor numbers
            for visitors in [1, 2]:
                logger.info(f"Testing with {visitors} visitors...")
                
                # Get ticket IDs
                ticket_ids = await self.monitor._get_ticket_ids(self.test_date, visitors)
                ...
                for ticket_id in ticket_ids:
                    ...
                    logger.info(f"Visitors={visitors}, TicketID={ticket_id}: 17:30 slot {'found' if slot_1730 else 'not found'}")
            
            self.results['direct_api'] = {'completed': True}
            
        except Exception as e:
            logger.error(f"❌ Direct API test failed: {e}")
            self.results['direct_api'] = {'error': str(e)}
